/**
 * Generate centroid-based H3 resolution-8 mapping for Hawaii State Parks
 *
 * Each of the 70 park features is mapped to a single res-8 H3 cell
 * via its polygon centroid, linking it to the existing moku backbone.
 *
 * Run: node parks/generate-parks-h3.mjs
 */
...
const INPUT_FILE = path.join(__dirname, 'State_Parks.geojson');
const OUTPUT_FILE = path.join(__dirname, 'PRK_Zones_H3.csv');
...
console.log('=== Hawaii State Parks — Centroid H3 Mapping ===\n');
...
for (const feature of geojson.features) {
  ...
  const provenance = `Centroid res8 from state park objectid ${oid} polygon`;

  rows.push(`${zoneId},${h3Cell},8,${islandNorm},${VERSION},State Parks centroid 2026,${provenance}`);
}
...
console.log(`Wrote ${rows.length} rows to PRK_Zones_H3.csv`);
