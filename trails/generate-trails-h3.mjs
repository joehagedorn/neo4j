/**
 * Generate midpoint-based H3 resolution-8 mapping for Hawaii Trails
 *
 * Each of the 45 trail features is mapped to a single res-8 H3 cell
 * via the midpoint of its LineString, linking to the existing moku backbone.
 *
 * Run: node trails/generate-trails-h3.mjs
 */
...
const INPUT_FILE = path.join(__dirname, 'Parks_Recreation_-4303987071572082682.geojson');
const OUTPUT_FILE = path.join(__dirname, 'TRL_Zones_H3.csv');
...
console.log('=== Hawaii Trails — Midpoint H3 Mapping ===\n');
...
for (const feature of geojson.features) {
  ...
  const oid = props.objectid_1;
  const island = props.island;
  const zoneId = `TRL_${oid}`;
  ...
  const provenance = `Midpoint res8 from trail objectid ${oid} linestring`;

  rows.push(`${zoneId},${h3Cell},8,${islandNorm},${VERSION},Parks Recreation trails midpoint 2026,${provenance}`);
}
...
console.log(`Wrote ${rows.length} rows to TRL_Zones_H3.csv`);
