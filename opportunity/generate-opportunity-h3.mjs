/**
 * Generate centroid-based H3 resolution-7 mapping for Hawaii Opportunity Zones
 *
 * Each of the 25 opportunity zone features is mapped to a single res-7 H3
 * cell via its polygon centroid, linking it to the existing moku backbone.
 *
 * Run: node opportunity/generate-opportunity-h3.mjs
 */
...
const INPUT_FILE = path.join(__dirname, 'Opportunity_Zones.geojson');
const OUTPUT_FILE = path.join(__dirname, 'OZ_Zones_H3.csv');
...
console.log('=== Hawaii Opportunity Zones — Centroid H3 Mapping ===\n');
...
for (const feature of geojson.features) {
  ...
  const tractName = props.tract_name;
  const zoneId = `OZ_${oid}`;
  ...
  const provenance = `Centroid res7 from OZ tract ${props.tract_no} (${tractName}) polygon`;

  rows.push(`${zoneId},${h3Cell},7,${VERSION},OZ centroid 2026,${provenance}`);
  console.log(`  ${zoneId}: ${tractName} -> ${h3Cell}`);
}
...
console.log(`\nWrote ${rows.length} rows to OZ_Zones_H3.csv`);
